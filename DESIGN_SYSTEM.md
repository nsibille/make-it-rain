...
- **Courbes** : `--ease` (sortie standard) · `--ease-out` (décélération, entrées de panneaux) · `--ease-spring` (léger rebond, `pop`). **Durées** (resserrées pour un ressenti vif) : `--duration-instant` 70 (pression) · `--duration-fast` 90 (hover/focus) · `--duration-base` 150 (fondus) · `--duration-slow` 230 (modales, volets). Plafond : rien au-delà de ~250 ms côté UI — le mouvement confirme, il ne fait jamais patienter.
